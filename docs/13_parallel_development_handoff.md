...
各スレッドは、作業終了時に未コミット変更を残さないことを原則とする。

- 実装、検証、docs更新まで終わったら、自分が触ったファイルだけをstageする。
- `git diff --cached` でstage内容を確認する。
- 説明できる単位でcommitする。
- GitHubへpushする。
- 完了報告にはcommit hash、push先、`npm run build` 結果、ブラウザ確認結果を書く。

ユーザーは、通常の開発作業についてエージェントが確認なしにcommit / pushすることを許可している。確認が必要なのは、破壊的操作、他担当変更の巻き込み、外部素材の公開、スコープ外機能追加、またはbuild失敗状態の共有が必要な場合だけ。

通常の並列開発は作業ブランチを使う。対話中の小さな修正や単独担当の調整で、すでに `main` 上にいる場合は、`npm run build` とブラウザ確認後に `main` へ直接commit / pushしてよい。
...
- 並列で大きく進める作業は `main` へ直接pushしない。PR経由で入れる。
- 対話中の小さな修正や単独担当の調整は、検証済みなら `main` へ直接commit / pushしてよい。
