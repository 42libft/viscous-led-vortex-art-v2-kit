...
- 白い発光リングとして加算するのではなく、原則として周囲模様のUV warpで周回帯を見せる。
...
- 黒円の黒はEvent Horizonの硬い境界で止め、重力場側へ黒が滲む表現にはしない。
- Event Horizonのサイズは黒円本体として安定させ、重力場やPhoton Ring調整のために肥大化させない。
- 進行方向正面の模様はローブ状・花びら状に分けず、黒円近傍の周回軌道へ沿わせて引き伸ばす。
- 後方の引きずりは尾を黒く塗るのではなく、周回軌道から抜けて流れるサンプリング座標の伸びとして見せる。
- 重力場には有限の外周境界を設け、その外側にはwarp / mask / 黒混ぜの影響を残さない。
- 重力場内を均一な円盤として歪ませず、周囲模様を円周方向の筋として拾い、黒円の前後方向で非対称に巻き込む。
