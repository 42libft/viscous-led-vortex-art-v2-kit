...
作業終了時は、原則としてGitHubへcommit / pushまで行う。これはユーザー確認待ちを減らすためではなく、並列開発中の各スレッドが最新状態を共有し、未コミット変更を溜め込まないための運用とする。

エージェントは、通常の開発作業について確認なしにcommit / pushしてよい。確認が必要なのは、破壊的操作、他担当変更の巻き込み、外部素材の公開、スコープ外機能追加、またはbuild失敗状態の共有が必要な場合だけ。
...
完了時は `npm run build` とブラウザ表示確認を行い、自分の変更だけをcommitしてGitHubへpushしてください。
```
