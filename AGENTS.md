...
- 作業終了時に、原則として未コミット変更を残さない。実装・検証・docs更新が終わったら、自分の変更だけをstageしてcommitし、GitHubへpushする。
- ユーザーは、通常の開発作業についてエージェントが確認なしにcommit / pushすることを許可している。確認が必要なのは、破壊的操作、外部素材の公開、スコープ外機能追加、他担当変更の巻き込みがある場合だけ。
- 並列開発の通常作業では、担当範囲ごとに短い作業ブランチを切る。
- 対話中の小さな修正や単独担当の調整で、すでに `main` 上にいる場合は、`npm run build` とブラウザ確認が通った後に `main` へ直接commit / pushしてよい。
